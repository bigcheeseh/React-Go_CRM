import React, {Component } from 'react';

class PhotoCell extends Component{
    state={
        imageUrl: {}
    }

    componentWillMount = () =>{
        const { original, auth } = this.props

        if(original.photo && original.photo.length > 0){
            this.props.fetchFile(original.photo, 'PHOTO', auth.token, original.id)
        }
    }
    componentWillReceiveProps=(nextProps)=>{
        const { original } = this.props
        if (nextProps.files.photo && nextProps.files.photo[original.id] && nextProps.files.photo[original.id] !== this.state.imageUrl[original.id]) {

            this.setState({ imageUrl: nextProps.files.photo })

        }
    }
    shouldComponentUpdate = (nextProps, nextState)=>{
        if(nextState.imageUrl !== this.state.imageUrl){
            return true
        }
        if (nextProps.original !== this.props.original) {
            return true
        }
        return true
    }
    render(){
        const { original } = this.props
 
        return (
            <div style={{ cursor: 'pointer' }}>
                <img
                    width="100%"
                    height="55px"
                    style={{ objectFit: 'cover' }}
                    src={this.state.imageUrl && this.state.imageUrl[original.id] ? this.state.imageUrl[original.id] : 'data:image/png;base64,'} />
            </div>
        )
    }
}

export default PhotoCell